...
defmodule LightController do
  ...
  defp turn_on(light_grid, range) do
    Enum.reduce(range, light_grid, fn(position, light_grid) ->
      brightness = Map.get(light_grid, position, 0)
      Map.put(light_grid, position, brightness + 1)
    end)
  end
  ...
  defp turn_off(light_grid, range) do
    Enum.reduce(range, light_grid, fn(position, light_grid) ->
      brightness = Map.get(light_grid, position, 0)
      Map.put(light_grid, position, max(brightness - 1, 0))
    end)
  end
  ...
  defp toggle(light_grid, range) do
    Enum.reduce(range, light_grid, fn(position, light_grid) ->
      brightness = Map.get(light_grid, position, 0)
      Map.put(light_grid, position, brightness + 2)
    end)
  end
end
